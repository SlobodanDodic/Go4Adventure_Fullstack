...
export class AuthService {
  ...
  async register(dto: AuthDtoRegister): Promise<Tokens> {
    const hashedPassword = await argon2.hash(dto.password);
    console.log("auth/register:" + hashedPassword);
  ...
  async login(dto: AuthDtoLogin) {
    ...
    const userInfo = { refreshToken: tokens.refresh_token, loggedUser: user.username, role: user.role };
  ...
  async updateAccount(token: string, dto: AuthDtoRegister) {
    const hashedPassword = await argon2.hash(dto.password);
